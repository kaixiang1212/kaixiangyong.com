...
{#if $showVerticalNavBar}
  <div
    class="fixed flex flex-col h-full items-center place-content-center right-10 z-10"
    on:mouseenter={show}
    on:mouseleave={hide}
  ...
  >
    {#each { length: sectionCount } as _, i}
      <button
        tabindex="-1"
        class="relative flex items-center justify-center h-6 w-6 mt-2 mb-2 rounded-3xl hover:bg-blue-500 hover:rounded-lg transition-all ease-linear cursor-pointer group shadow-lg"
        on:click={() => onClick(i)}
