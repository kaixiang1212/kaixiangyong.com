...
<Card>
  <article>
    <header>
      <h2 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white left-0 w-full">Where I've Been</h2>
    </header>
        ...
        {#each experiences as experience, i (experience.name)}
          <TabPanel let:selected={active}>
            ...
            {#if (experience?.comment)}
              <p class="font-light text-gray-500 dark:text-gray-500 text-sm md:text-base transition-all"><em>{experience.comment}</em></p>
            {/if}
